...
def load_tradeoff_data() -> tuple[pd.DataFrame, pd.DataFrame]:
	...
	ppl_raw = pd.read_csv("../save/W4A5_format_block_size/results.tsv", sep="\t")
	...
	int5_row = area_results.loc[area_results["Precision"] == "W4A5_INT"].iloc[0]
	hw_data["int5"] = _block_series_from_row(int5_row, BLOCK_SIZES)

	w4a5_area = pd.read_csv("../hardware/BQ/MAC_EM/W4A5_area_results.tsv", sep="\t")
	for _, row in w4a5_area.iterrows():
	...
	e2_row = activation_area[
		...
		& (activation_area["activation_precision"] == 5)
	].iloc[0]
	hw_data["e2m2"] = _block_series_from_columns(e2_row, BLOCK_SIZES)
...
def plot_tradeoff(ppl_df: pd.DataFrame, hw_df: pd.DataFrame, out_path: str) -> None:
	...
	legend = ax.legend(title="A5 Format", frameon=True, prop={"weight": "bold", "size": 12}, title_fontproperties={"weight": "bold", "size": 12})
...
if __name__ == "__main__":
	...
	out_path = os.path.join(base_dir, "fig5.png")
