...
module FP_WXAY_vector (
    ...
    output reg  [FX_PSUM_WIDTH-1:0] PSUM,
    ...
    logic signed [L2_PSUM_WIDTH+3-1:0] sub_block_psum_scaled_x[0:NUM_SUB_B-1];
    logic signed [SCALED_L2_PSUM_WIDTH*NUM_SUB_B-1:0] scaled_l2_psum;
    logic signed [FX_PSUM_WIDTH-1:0] psum;
    ...
    // reg signed [PRODUCT_WIDTH+SUB_BLOCK_LEVEL:0]   l1_psum[0:1];
    // reg signed [PRODUCT_WIDTH+SUB_BLOCK_LEVEL+1:0] l2_psum;
    // // reg [(PRODUCT_WIDTH+LEVEL)*NUM_SUB_B-1:0] psum_reg;
    // reg signed [PRODUCT_WIDTH+LEVEL-1:0] psum_reg_w [0:NUM_SUB_B-1];
    // reg signed [PRODUCT_WIDTH+LEVEL-1:0] psum_reg_r [0:NUM_SUB_B-1];
    ...
    always_comb begin
        ...
        case (mode) 
            MODE_128: begin
                for (int sub_b = 0; sub_b < NUM_SUB_B; sub_b+=1) begin
                    scaled_l2_psum[sub_b*SCALED_L2_PSUM_WIDTH +: SCALED_L2_PSUM_WIDTH] = sub_block_psum_r[sub_b];
                end
            end
            MODE_L2: begin
                for (int sub_b = 0; sub_b < NUM_SUB_B; sub_b+=1) begin
                    ...
                    if (l2_scale_w_scheme == L2_SCHEME_FP) begin
                        ...
                        scaled_l2_psum[sub_b*SCALED_L2_PSUM_WIDTH +: SCALED_L2_PSUM_WIDTH] = (sub_block_psum_scaled_x[sub_b] * $signed({3'b010, l2_scale_w[sub_b*L2_SCALE_W_BIT +: L2_SCALE_W_BIT]}));
                    end
                end
            end
        endcase
